...
const has_ktp = (string) => {
  return (/\d{3}-\d\d-\d{4}/.test(string))
}
...
const grab_ktp = (string) => {
  let regX = /\d{3}-\d\d-\d{4}/.exec(string)
  if(regX === null)
    return null
  else
    return regX[0]
...
const grab_all_nomor_ktp = (string) => {
  return string.match(/\d{3}-\d\d-\d{4}/g)
}
...
console.log(format_nomor('234601422, 350.80.0744, 013-60-8762')) // true
