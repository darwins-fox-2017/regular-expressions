...
const has_ktp = (string) => {
  // ...
  let ktpPat = /(\d{3})-(\d{2})-(\d{4})/g; 
  return ktpPat.test(string)
}
...
const grab_ktp = (string) => {
  // ...
  let ktpPat = /(\d{3})-(\d{2})-(\d{4})/g; 
  let grabbed = string.match(ktpPat);
  return grabbed != null ? grabbed[0] : null; 
}
...
const grab_all_nomor_ktp = (string) => {
  // ...
  let ktpPat = /(\d{3})-(\d{2})-(\d{4})/g; 
  let grabbed = string.match(ktpPat) === null ? [] : string.match(ktpPat)
  return grabbed
}
...
const hide_all_nomor_ktp = (string) => {
  // ...
  let ktpPat = /(\d{3})-(\d{2})/g; 
  return string.replace(ktpPat, `XXX-XX`)
}
...
const format_nomor = (string) => {
  // ...
  let patt = /(\d{3})\W*(\d{2})\W*(\d{4})/g;

  if(patt.test(string)) {
    return string.replace(patt, '$1-$2-$3')
  }
}
