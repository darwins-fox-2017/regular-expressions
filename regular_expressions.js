...
const grab_ktp = (string) => {
   //var angka = string.match(/[^a-z\s]+$/gi)[0];
   var angka = string.match(/\d{3}-\d{2}-\d{4}/);
   if (angka) {
   	return angka[0];
   } else {
   	return null;
...
const grab_all_nomor_ktp = (string) => {
  let angka = string.match(/\d{3}-\d{2}-\d{4}/gi);
  if (angka) {
  	return angka;	
  } else {
  	return [];
...
const format_nomor = (string) => {
  let kode1 = string.match(/(\d{3}).??(\d{2}).??(\d{4})/g);
  
  // return kode1;
  let polaKode = /(\d{3}).??(\d{2}).??(\d{4})/g
  let tampungHasil = string.match(polaKode)
  
  if(tampungHasil){
    return string.replace(polaKode,`$1-$2-$3`);
  }else {
    return string;
  }
}
