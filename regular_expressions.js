...
const grab_ktp = (string) => {
  let result = string.match(/\d{3}-\d{2}-\d{4}/)
  if(result){
    return result[0]
  }else {
    return result
...
const hide_all_nomor_ktp = (string) => {
  return string.replace(/(\d{3})-(\d{2})-(\d{4})/g,"XXX-XX-$3")
}
...
var str = 'please confirm your identity: 44211422'
console.log(format_nomor(str) === str) // true
